...
public class StateMachine<TEnum> {
	...
	private bool firstStateTriggered = false;
	#endregion
	...
	bool IsAllowableState(int value) {
		bool isTriggered = (value != (int)(object)_currentState || !firstStateTriggered);
		firstStateTriggered = true;
		return isTriggered;
	}
	...
	void TriggerStateEnter() {
		...
		OnStateStart += _states[(int)(object)_currentState].OnStart;
